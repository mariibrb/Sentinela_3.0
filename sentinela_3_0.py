...
def carregar_clientes_ativos():
    ...
    if os.path.exists(caminho_lista):
        try:
            ...
            col_cod = next((c for c in df.columns if any(k in c for k in ['COD', 'ID'])), df.columns[0])
            col_nome = next((c for c in df.columns if any(k in c for k in ['NOME', 'CLIENTE', 'RAZAO'])), df.columns[1])
...
# 6. CORPO DA PÁGINA - DIVISÃO CLARA ENTRE ORIGEM E DOMÍNIO
if empresa_ok and (regime_ok if 'regime_ok' in locals() else False):
    ...
    # Abas agora focadas apenas na conferência do que está na Domínio
    ...
    if st.button("🚀 EXECUTAR FECHAMENTO COMPLETO", use_container_width=True):
        if xmls:
            with st.spinner("Auditando Origem e Conferindo Domínio..."):
                try:
            ...
            st.warning("⚠️ O Bloco 1 (XML) é obrigatório para a auditoria.")
